...
if(preg_match('/\.NET/i',$serverInfo)) { 
  ...
  $screenType = 'browser-window'; 
}
?>
<!DOCTYPE <html>
<html class="<?php echo $screenType; ?>">
<head>
...
<body class="">
	<div id="container" class="container">
		...
		<div class="copy-url col-sm-8 col-sm-offset-2">
			<label for="<?= $install_dir ?>">Local Wordpress URL:</label>
			<input type="text" id="<?= $install_dir ?>" placeholder="Search for..." name="<?= $install_dir ?>" value="http://<?= $base_url ?>/<?= $install_dir ?>/">
			<button type="button" onclick="copyit('<?= $install_dir ?>', '<?= $install_dir ?>')" name="copy_button" style="">Copy WordPress URL to Clipboard</button>
		</div>
		<div class="wrapper col-sm-8 col-sm-offset-2">
			...
			<? } ?>
			<div class="col-sm-12 creds">
				<p class="col-sm-12 text-center"><b>Default Credentials</b></p>
				<p class="col-sm-6 text-center"><u>WordPress</u><br>User = admin<br>Pass = password</p>
				<p class="col-sm-6 text-center"><u>MySQL</u><br>User = root<br>Pass =</p>
			</div>
		</div>
		<div class="wrapper col-sm-8 col-sm-offset-2">
			...
			<?php
				$dir = '../';
					foreach(scandir($dir) as $entry) {
						if ($entry != 'wordpress' && $entry != 'instantwp' && !preg_match("/(.*\..*)/", $entry)) {
